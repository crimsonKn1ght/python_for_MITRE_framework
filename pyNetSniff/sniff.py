...
def ExtractTelnet(packet):
    try:
        ...
        return
    connData = [packet[IP].src, packet[TCP].sport]  #Assuming server is source
    if payload[:5] == "login":
        ...
        return
    connData = [packet[IP].src, packet[TCP].dport] #Assuming client is source
...
packets = rdpcap("overpass2.pcapng")
